...
def ftc(request):
    ...
    if current.is_staff:
        if request.method =='POST':
            ...
            return render (request,"appointments/show2.html",{
                "list2": c
            })
        else:
...
def dept(request):
    return render(request,"appointments/show1.html",{
...
def contact(request):
    return render (request,"appointments/layout.html",{
        "co": f"Problem ?",
        "nt": f"You may contact through the mail or the below listed phone number to get your problems solved!",
        "ac":  f"Mail: medezeehelp@gmail.com ",
        "ct":  f"Dail : 1800-11-1363 ",
